jQuery(document).ready(function($){
    ...
    $('input#holiday_id', parent).change(function(event) {
        var holiday_id = $(this).val();
        if(typeof holiday_id != 'undefined' && parseInt(holiday_id) > 0){
            $('#overlay', parent).addClass('active');
            data = {
                action: 'st_getInfoHoliday',
                holiday_id: holiday_id
            };
            $.post(st_params.ajax_url, data, function(respon, textStatus, xhr) {
                $('#overlay', parent).removeClass('active');
                if(typeof respon == 'object'){
                    $('#type-holiday-wrapper', parent).html(respon.type_holiday);
                    $('input#max_people', parent).val(respon.max_people);
                    $('input#adult_price', parent).val(respon.adult_price);
                    $('input#child_price', parent).val(respon.child_price);
                    $('input#infant_price', parent).val(respon.infant_price);
        ...
        this.initCalendar = function(){
            self.calendar.fullCalendar({
                ...
                events:function(start, end, timezone, callback) {
                    $.ajax({
                        ...
                        data: {
                            ...
                            holiday_id: $('input#holiday_id').val(),
                            start: start.unix(),
                            end: end.unix()
                        },
                        success: function(doc){
                ...
                eventRender: function(event, element, view){
                    ...
                    element.addClass('event-number-'+event.start.unix());
                    element.bind('dblclick', function() {
                        $('input#check_in_holiday').val(event.start._i);
                        if(typeof event.end != 'undefined' && event.end && typeof event.end._i != 'undefined'){
                                date = new Date(event.end._i);
                                date.setDate(date.getDate() - 1);
                                date = $.fullCalendar.moment(date).format("YYYY-MM-DD");
                            $('input#check_out_holiday').val(date).parents('.form-group').show();
                        }else{
                            $('input#check_out_holiday').val(event.start._i).parents('.form-group').hide();
                        }
                        $('input#adult_price').val(event.adult_price);
                        $('input#child_price').val(event.child_price);
                        $('input#infant_price').val(event.infant_price);
                        $('#holiday_time').qtip('hide');
                    });
                },
                loading: function(isLoading, view){
                    if(isLoading){
    ...
    $('input#check_out_holiday').parents('.form-group').hide();
    
    if($('#holiday_time').length){
        $('#holiday_time').qtip({
            content: {
                text: $('#holiday_time_content').html()
            },
            show: { 
                when: 'click', 
                solo: true // Only show one tooltip at a time
