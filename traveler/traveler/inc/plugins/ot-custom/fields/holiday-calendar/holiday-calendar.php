<?php 
if(!class_exists('ST_Holiday_Calendar_Field')){
	class ST_Holiday_Calendar_Field{
		public  $url;
        public $dir;

        function __construct(){

            $this->dir = st()->dir('plugins/ot-custom/fields/holiday-calendar');
            $this->url = st()->url('plugins/ot-custom/fields/holiday-calendar');
        ...
        function init(){
            ...
            add_filter( 'ot_st_holiday_calendar_unit_types', array($this, 'ot_post_select_ajax_unit_types'), 10, 2 );

            add_filter( 'ot_option_types_array', array($this, 'ot_add_custom_option_types'));

        }
        function add_scripts(){
            if(get_post_type() == 'st_holidays'){
        ...
        function ot_post_select_ajax_unit_types($array, $id){
            return apply_filters( 'st_holiday_calendar', $array, $id );
        }

        function ot_add_custom_option_types( $types ) {
            $types['st_holiday_calendar'] = __('Availability holiday',ST_TEXTDOMAIN);
    ...
    $calendar_holiday = new ST_Holiday_Calendar_Field();
    $calendar_holiday->init();

    if(!function_exists('ot_type_st_holiday_calendar')){
        function ot_type_st_holiday_calendar($args = array()){
            $calendar_holiday = new ST_Holiday_Calendar_Field();
            $default = array(
                'field_name' => ''
            );
            ...
            echo $calendar_holiday->load_view(false, $args);
        }
    }    
}
?>