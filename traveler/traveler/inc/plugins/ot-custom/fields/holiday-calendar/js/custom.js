jQuery(function($){
			...
			self.calendar.fullCalendar({
				...
				events:function(start, end, timezone, callback) {
                    $.ajax({
                        ...
                        type:'post',
                        data: {
                            action: 'st_get_availability_holiday',
	...
	jQuery(document).ready(function($) {
		$('.calendar-wrapper').each(function(index, el) {
			...
			$('#calendar_submit', t).click(function(event) {
				var data = $('input, select', '.calendar-form').serializeArray();
					data.push({
						name: 'action',
						value: 'st_add_custom_price_holiday'
					});
            ...
            $(document).on('click','.ui-tabs-anchor[href=#setting_availability_tab]',function(){
                holiday.calendar.fullCalendar( 'refetchEvents' );
            });
		...
		if($('select#type_holiday').length && $('select#type_holiday').val() == 'daily_holiday'){
			$('input#calendar_groupday').prop('checked', false).parent().hide();
		}else{
			$('input#calendar_groupday').parent().show();
		}
		$('select#type_holiday').change(function(event) {
			holiday_type = $(this).val();
