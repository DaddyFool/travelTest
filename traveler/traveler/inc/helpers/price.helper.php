...
if(!class_exists('STPrice')){
    class STPrice{
         ...
         * use for holiday
         **/
        static function getPeoplePrice($holiday_id, $check_in, $check_out){
            global $wpdb;
            $data_price = array(
                'adult_price' => 0,
                'child_price' => 0,
            ...
            if(is_array($data) && count($data)){
                ...
                if($type_holiday == 'daily_holiday'){
                    $data_price['adult_price'] = floatval(get_post_meta($holiday_id, 'adult_price', true));
                    $data_price['child_price'] = floatval(get_post_meta($holiday_id, 'child_price', true));
                    $data_price['infant_price'] = floatval(get_post_meta($holiday_id, 'infant_price', true));
                }
            }

            return $data_price;
        }
        static function getPriceByPeopleHoliday($holiday_id = '', $check_in = '', $check_out = '', $adult_number = 0, $child_number = 0, $infant_number = 0){
            $total_price = 0;

            $holiday_id = intval($holiday_id);

            $people_price = self::getPeoplePrice($holiday_id, $check_in, $check_out);
            ...
            $discount_by_adult = get_post_meta($holiday_id, 'discount_by_adult', true);
            $discount_by_child = get_post_meta($holiday_id, 'discount_by_child', true);
        ...
        static function getSaleHolidaySalePrice($post_id = '', $price = '', $holiday_type = '', $check_in = ''){
            $total_price = 0;
            ...
            if(is_array($cart) && count($cart)){
                foreach($cart as $key => $val){
                    ...
                    if(get_post_type($post_id) == 'st_holidays'){
                        ...
                        $child_number = intval($val['data']['child_number']);
                        $infant_number = intval($val['data']['infant_number']);
                        $data_prices = self::getPriceByPeopleHoliday($post_id, strtotime($check_in), strtotime($check_out), $adult_number, $child_number, $infant_number);
                        $origin_price = floatval($data_prices['total_price']);

                        $holiday_type = $val['data']['type_holiday'];

                        $sale_price = STPrice::getSaleHolidaySalePrice($post_id, $origin_price, $holiday_type, strtotime($check_in));
                        $price_with_tax = self::getPriceWithTax($sale_price);
                        $coupon_price = self::getCouponPrice();
                        $deposit_price = self::getDepositPrice($val['data']['deposit_money'], $price_with_tax, $coupon_price);
                        if(isset($val['data']['deposit_money'])){
        ...
        static function getDataPrice(){
            ...
            if(is_array($cart) && count($cart)){
                foreach($cart as $key => $val){
                    ...
                    if(get_post_type($key) == 'st_holidays'){
                        $post_id = intval($key);
                        $check_in = $val['data']['check_in'];
                        $check_out = $val['data']['check_out'];
                        $adult_number = intval($val['data']['adult_number']);
