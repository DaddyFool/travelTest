...
if(!class_exists('ValidateWooCheckout')){
	class ValidateWooCheckout{
		...
		static function get_cart_data($cart, $post_type){
			...
			if($post_type == 'st_holidays' && count($cart)){
				foreach($cart as $key => $val){
					if($val['st_booking_data']['st_booking_post_type'] == 'st_holidays'){
						$data[$val['st_booking_data']['st_booking_id']]['date'][] = array(
							'check_in' => strtotime($val['st_booking_data']['check_in']),
							'check_out' => strtotime($val['st_booking_data']['check_out']),
						);
						$data[$val['st_booking_data']['st_booking_id']]['type_holiday'] = $val['st_booking_data']['type_holiday'];
						$data[$val['st_booking_data']['st_booking_id']]['people'][] = intval($val['st_booking_data']['adult_number']) + intval($val['st_booking_data']['child_number']) + intval($val['st_booking_data']['infant_number']);
						if(!isset($data[$val['st_booking_data']['st_booking_id']]['min']) && !isset($data[$val['st_booking_data']['st_booking_id']]['max'])){
							$data[$val['st_booking_data']['st_booking_id']]['min'] = strtotime($val['st_booking_data']['check_in']);
							$data[$val['st_booking_data']['st_booking_id']]['max'] = strtotime($val['st_booking_data']['check_out']);
		...
		static function check_validate_holiday($data){
			if(is_array($data) && count($data)){
				foreach($data as $key => $val){
					$holiday_id = $key;
					...
					$result = self::_get_order_holidays($key, $val['min'], $val['max'], $val['type_holiday']);
					for($i = intval($val['min']); $i <= intval($val['max']); $i = strtotime('+1 day', $i) ){
						$people = 0;
						$people_cart = 0;
						if(is_array($result) && count($result)){
							foreach($result as $item){
								if($val['type_holiday'] == 'specific_date' && $i >= intval($item['check_in_timestamp']) && $i <= intval($item['check_out_timestamp'])){
									$people += intval($item['people']);
								}
								if($val['type_holiday'] == 'daily_holiday' && $i == intval($item['check_in_timestamp'])){
									$people += intval($item['people']);
								}
							}
							foreach($val['date'] as $k => $date){
								if($val['type_holiday'] == 'specific_date' && $i >= intval($date['check_in']) && $i <= intval($date['check_out'])){
									$people_cart += intval($val['people'][$k]);
								}
								if($val['type_holiday'] == 'daily_holiday' && $i == intval($date['check_in'])){
									$people_cart += intval($val['people'][$k]);
								}
							}
							if($people_cart + $people > $max_people){
								...
								$_SESSION['flash_validate_checkout'] = '<p class="bg-danger" style="padding: 10px 5px">'.sprintf(__('The <b>%s</b> holiday has only %s people available on <b>%s</b>.',ST_TEXTDOMAIN), get_the_title($holiday_id), $free_people, date(TravelHelper::getDateFormat(),$i)).'</p>';
								return false;
							}
						}else{
							$people_cart = 0;
							...
							if($people_cart > $max_people){
								$_SESSION['flash_validate_checkout'] = '<p class="bg-danger" style="padding: 10px 5px">'.sprintf(__('This <b>%s</b> holiday has only %s people available on <b>%s</b>.',ST_TEXTDOMAIN), get_the_title($holiday_id), ($max_people), date(TravelHelper::getDateFormat(),$i)).'</p>';
								return false;
							}
						} 
		...
		static function _get_order_holidays($holiday_id, $check_in, $check_out, $holiday_type = 'daily_holiday'){
			global $wpdb;
			...
			WHERE mt.type_holiday = '{$holiday_type}'
			AND
