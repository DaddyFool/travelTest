	...
	class AvailabilityHelper{
		public function __construct(){
			if(is_admin()){
				...
				add_action('wp_ajax_st_get_availability_holiday', array(&$this, '_get_availability_holiday'));
				add_action('wp_ajax_st_get_availability_holiday_frontend', array(&$this, '_get_availability_holiday_frontend'));
				add_action('wp_ajax_nopriv_st_get_availability_holiday_frontend', array(&$this, '_get_availability_holiday_frontend'));
				...
				add_action('wp_ajax_st_add_custom_price_holiday', array(&$this, '_add_custom_price_holiday'));
			}
		}
		...
		public function _get_availability_holiday(){
			$results = array();
			...
			if(get_post_type($holiday_id) == 'st_holidays'){
				...
				$data_holiday = self::_getdataTourEachDate($holiday_id, $check_in, $check_out);
				if(is_array($data_holiday) && count($data_holiday)){
					...
					if($type_holiday == 'daily_holiday' || !$type_holiday){
						for($i = $check_in; $i <= $check_out; $i = strtotime('+1 day', $i)){
							$results[] = array(
								'title' => get_the_title($holiday_id),
								'start' => date('Y-m-d',$i),
		...
		public function _get_availability_holiday_frontend(){
			$results = array();
			$holiday_id = STInput::request('holiday_id', '');
			$check_in = STInput::request('start', '');
			$check_out = STInput::request('end', '');
		...
		static function _getdataHolidayEachDate($holiday_id, $check_in, $check_out){
			global $wpdb;
		...
		public function _add_custom_price_holiday(){
			global $wpdb;
			$check_in = STInput::request('calendar_check_in', '');
			$check_out = STInput::request('calendar_check_out', '');
			if(empty($check_in) || empty($check_out)){
			...
			$groupday = STInput::request('calendar_groupday', '');
			for($i = $check_in; $i <= $check_out; $i = strtotime('+1 day', $i)){
				$list_holiday = self::_getdataHolidayEachDate($post_id, $i, $i);
				if(is_array($list_holiday) && count($list_holiday)){
					foreach($list_holiday as $key => $val){
						if($i == $val->check_in){
							if(intval($val->check_out) - intval($val->check_in) == 0){
								self::_deleteData($val->id);
							}else{
