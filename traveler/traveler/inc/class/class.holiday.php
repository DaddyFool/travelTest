<?php
     ...
     *
     */
    if(!class_exists('STHoliday'))
    {
        class STHoliday extends TravelerObject
        {
             ...
             * @since 1.1.7
             */
            protected $template_folder='holidays';
            function __construct($holidays_id=false)
            {
                $this->orderby=array(
                    'new'=>array(
                        'key'=>'new',
                    ...
                    'name_a_z'=>array(
                        'key'=>'name_a_z',
                        'name'=>__('Holidays Name (A-Z)',ST_TEXTDOMAIN)
                    ),
                    'name_z_a'=>array(
                        'key'=>'name_z_a',
                        'name'=>__('Holidays Name (Z-A)',ST_TEXTDOMAIN)
                    )
                );
            ...
            {
                return st()->get_option('holiday_sidebar_pos', 'left');
            }
            /**
             * @since 1.1.9
             * @return bool
            ...
            {
                $review_stat = st()->get_option( 'holiday_review_stats' );
            ...
            {
                echo st()->load_template('holidays/wc_cart_item_information',false,array('st_booking_data'=>$st_booking_data));
            } 
            /**
             *
             *
            ...
            {
                echo '<span class="booking-item-wishlist-title"><i class="fa fa-flag-o"></i> '.__('holiday',ST_TEXTDOMAIN).' <span></span></span>';
            }


            function _st_data_custom_price(){
                return array('title'=>'Price Custom Settings','post_type'=>'st_holidays');
            }
            ...
            static function get_search_fields_name()
            {
                return array(
                    ...
                    'item_name'=>array(
                        'value'=>'item_name',
                        'label'=>__('Holiday Name',ST_TEXTDOMAIN)
                    ),
                    'list_name'=>array(
                        'value'=>'list_name',
            ...
            {
                if( get_query_var('post_type')=='st_holidays')
                {
                    $return=__(" Holidays in %s",ST_TEXTDOMAIN);
                    ...
                    if(STInput::get('location_id'))
                    {
                        ...
                        $return=__(" Holidays",ST_TEXTDOMAIN);
                    }
            ...
            function add_sidebar()
            {
                register_sidebar( array(
                    'name' => __( 'Holidays Search Sidebar 1', ST_TEXTDOMAIN ),
                    'id' => 'holidays-sidebar',
                    'description' => __( 'Widgets in this area will be shown on Holidays', ST_TEXTDOMAIN),
                    'before_title' => '<h4>',
                    'after_title' => '</h4>',
                ...
                register_sidebar( array(
                    'name' => __( 'Holiday Single Sidebar', ST_TEXTDOMAIN ),
                    'id' => 'holiday-single-sidebar',
                    'description' => __( 'Widgets in this area will be shown on all holiday.', ST_TEXTDOMAIN),
                    'before_title' => '<h4>',
                    'after_title' => '</h4>',
                    'before_widget' => '<div id="%1$s" class="sidebar-widget %2$s">',
                    'after_widget'  => '</div>',
            ...
            **/
            ...
            function _alter_search_query($where)
            {
                ...
                // only daily holiday
                // check if holiday search result
//                $where.=" AND {$wpdb->posts}.ID in
//                            (SELECT post_id
//                                from $wpdb->postmeta
//                                join $wpdb->posts as mt10 on  {$wpdb->postmeta}.post_id  = mt10.ID
//                                where meta_key='type_holiday' and meta_value='daily_holiday'
//                                and mt10.post_type = 'st_holidays'
//                                )";
            ...
            function _alter_join_query($JOIN)
            {
                return $JOIN;
                global $wpdb;
                //$JOIN.="LEFT JOIN $wpdb->postmeta as st_meta1 on st_meta1.post_id=$wpdb->posts.ID and st_meta1.meta_key='type_holiday' and st_meta1.meta_key='daily_holiday'";
                return $JOIN;
            }

            function _get_join_query($join)
            {
                if(!TravelHelper::checkTableDuplicate('st_holidays')) return $join;
            ...
            */
            function _get_where_query_tab_location($where){
                ...
                if(!TravelHelper::checkTableDuplicate('st_holidays')) return $where;
            ...
            */
            ...
            {
                remove_action('pre_get_posts',array($this,'change_search_holiday_arg'));
             ...
             * @update 1.1.3
             * */
            function change_search_holiday_arg($query)
            {
                ...
                if($query->is_search && $post_type == 'st_holidays')
                {
                    ...
                    $is_featured = st()->get_option('is_featured_search_holiday','off');
            ...
            function choose_search_template($template)
            {
                global $wp_query;
                $post_type = get_query_var('post_type');
                if( $wp_query->is_search && $post_type == 'st_holidays' )
                {
                    return locate_template('search-holiday.php');  //  redirect to archive-search.php
                }
                return $template;
            }
            ...
            static function get_count_user_book($post_id=null){
                ...
                if(st()->get_option('use_woocommerce_for_booking','off') == 'on'){
                    ...
                    $type_holiday = get_post_meta($post_id , 'type_holiday' , true);
            ...
            */
            ...
            function get_cart_item_html($item_id = false){
                return st()->load_template('holidays/cart_item_html',null,array('item_id'=>$item_id));
            }

            function custom_holiday_layout($old_layout_id)
            {
                if(is_singular('st_holidays'))
                {
            ...
            {
                $fields=st()->get_option('activity_holiday_search_fields');
                return $fields;
            }
            ...
            {
                $this->post_type='st_holidays';
                //$limit = st()->get_option('holidays_similar_holiday',5);
            ...
            public static function holiday_external_booking_submit(){
                /*
                 * since 1.1.1
                 * filter hook holiday_external_booking_submit
                */
                $post_id = get_the_ID();
                if (STInput::request('post_id')) {$post_id = STInput::request('post_id') ; }

                $holiday_external_booking = get_post_meta($post_id, 'st_holiday_external_booking' , "off");
                $holiday_external_booking_link = get_post_meta($post_id , 'st_holiday_external_booking_link' ,true) ;
                if ($holiday_external_booking =="on" and $holiday_external_booking_link!==""){
                    if (get_post_meta($post_id , 'st_holiday_external_booking_link' , true)){
                        ob_start();
                        ?>
                            <a class='btn btn-primary' href='<?php echo get_post_meta($post_id , 'st_holiday_external_booking_link' , true) ?>'> <?php st_the_language('book_now')  ?></a>
                        <?php
                    $return  =  ob_get_clean();
                    }
                }
                ...
                return apply_filters('holiday_external_booking_submit' , $return ) ;
            }

            /* @since 1.1.3 */
            static function get_taxonomy_and_id_term_holiday()
            {
