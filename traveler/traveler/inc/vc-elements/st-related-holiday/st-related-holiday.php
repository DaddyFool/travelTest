...
if(function_exists( 'vc_map' )) {
    ...
    vc_map( array(
        "name"            => __( "ST List Holiday related" , ST_TEXTDOMAIN ) ,
        "base"            => "st_list_holiday_related" ,
...
if(!function_exists( 'st_list_holiday_related' )) {
    function st_list_holiday_related( $attr , $content = false )
    {
        ...
        $data    = shortcode_atts(
            $param , $attr , 'st_list_holiday_related' );
        extract( $data );
...
if(st_check_service_available( 'st_holidays' )) {
    st_reg_shortcode( 'st_list_holiday_related' , 'st_list_holiday_related' );
}