...
<form id="hotel_search_half_map" method="get" class="search filter_search_map" action="<?php echo home_url() ?>">
    ...
    <div class="row">
        ...
        if(!empty( $fields )) {
            foreach( $fields as $key => $value ) {
                ...
                ?>">
                    <?php echo st()->load_template( 'holidays/elements/search/field' , $name , array( 'data'          => $value ,
    ...
    <!--<div class="form-group price_map">
        <?php
/*        $data_min_max = TravelerObject::get_min_max_price( 'st_holidays' );
    ...
    <button class="btn btn-primary btn_search" data-title="<?php st_the_language( 'search_for_holiday' ) ?>"
            type="submit"><?php st_the_language( 'search_for_holiday' ) ?></button>
    <hr>
</form>
