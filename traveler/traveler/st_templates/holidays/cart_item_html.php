...
if(isset($item_id) and $item_id):
    ...
    $type_holiday=$item['data']['type_holiday'];
    ...
    <header class="clearfix" style="position: relative">
        ...
        <?php } ?>
        <?php if(get_post_status($holiday_id)):?>
        <a class="booking-item-payment-img" href="#">
            <?php echo get_the_post_thumbnail($holiday_id,array(98,74,'bfi_thumb'=>true));?>
        </a>
        <h5 class="booking-item-payment-title"><a href="<?php echo get_permalink($holiday_id)?>"><?php echo get_the_title($holiday_id)?></a></h5>
        <ul class="icon-group booking-item-rating-stars">
            <?php echo TravelHelper::rate_to_string(STReview::get_avg_rate($holiday_id)); ?>
        </ul>
        <?php
        else: st_the_language('sorry_holiday_not_found');
        endif;?>
    </header>
    <ul class="booking-item-payment-details">
        <li>
            <h5><?php st_the_language('holidays_information')?></h5>
            ...
            <ul class="booking-item-payment-price">
                <li>
                    <p class="booking-item-payment-price-title">
                        <?php echo __('Type Holiday', ST_TEXTDOMAIN); ?>
                    </p>
                    <p class="booking-item-payment-price-amount">
                        <?php 
                            if($type_holiday == 'daily_holiday'){
                                echo __('Daily Holiday', ST_TEXTDOMAIN);
                            }elseif($type_holiday == 'specific_date'){
                ...
                <?php endif; ?>
                <?php if($type_holiday == 'daily_holiday' and $duration): ?>
                <li>
                ...
                <?php endif; ?>
                <!-- <li>
                    <p class="booking-item-payment-price-title"><?php  st_the_language('holiday_max_people')?> </p>
...
<div class="booking-item-payment-total text-right">
    <?php
        ...
        $sale_price = STPrice::getSaleHolidaySalePrice($item_id, $origin_price, $type_holiday, strtotime($check_in));
