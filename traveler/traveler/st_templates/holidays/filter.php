...
$default=array(
    'title'=>st_get_language('holiday_filter_by').':',
    'st_search_fields'=>'',
    'style'         =>'dark'
);
...
?>
<aside class="booking-filters <?php if($style=='dark') echo 'text-white'; else echo 'booking-filters-white';?> holidays-filters">
        ...
        if(!empty($all_fields)and is_array($all_fields)):
                foreach($all_fields as $key=>$value):
                    ...
                    echo ' <h5 class="booking-filters-title">'.apply_filters( 'widget_title' , $value->title).'</h5>';
                        switch($value->field){
                            case "price":
                                echo st()->load_template('holidays/filter_price');
                                break;
                            case "rate":
                                    $max_rate=5;
                                        ...
                                        <div class="checkbox">
                                            <label>
                                                ...
                                                <ul class="icon-group search_rating_star">                                                        <?php $i_s='<li><i class="fa fa-star"></i></li>';
                                                    ...
                                                    ?>
                                                    <?php /*$count=STHotel::count_meta_key('rate_review',$i,'st_holidays');
                                                    echo "&nbsp;({$count})";*/?>
                                               </ul>
                                            </label>
                                        </div>
...
<script>
    jQuery(document).ready(function($){
        $('.holidays-filters input[type=checkbox]').on('ifClicked', function(event){
