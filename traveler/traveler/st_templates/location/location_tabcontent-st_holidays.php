<?php 
    ...
    if ($st_location_style =="list"){
        $return .='<ul class="booking-list loop-holidays style_list">' ; 
    }else {
    ...
    add_filter('posts_join' , array($holiday , '_get_join_query'));
    add_filter('posts_where' , array($holiday , '_get_where_query_tab_location'));

    query_posts($query);    
    
    remove_filter('posts_join' , array($holiday , '_get_join_query'));
    remove_filter('posts_where' , array($holiday , '_get_where_query_tab_location'));
    if (have_posts()){
        while(have_posts()){
            the_post();
            $return .=st()->load_template('holidays/elements/loop/loop-1',null);
        }
    }else {
        echo '<div class="col-xs-12"><div class="alert alert-warning">'.__("There are no available holiday for this location, time and/or date you selected.",ST_TEXTDOMAIN).'</div></div>';
    }
