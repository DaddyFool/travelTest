...
/**
*@since 1.1.9
* use for Booking Holiday
**/
?>
<div class="st-create">
    <h2><?php _e("Add New Holiday Booking",ST_TEXTDOMAIN) ?></h2>
</div>
      			...
      			<div class="panel-body">
	      			<div class="row">
	      				<div class="col-xs-12">
	      					<div class="form-group">
						        <label for="holiday_id" class="head_bol"><?php _e('Holiday', ST_TEXTDOMAIN); ?> <span class="text-small text-danger">(*)</span>:</label>
						        <?php 
			                        $holiday_id = isset($_POST['item_id']) ? $_POST['item_id'] : '';
			                        $holiday_name = (intval($holiday_id) > 0) ? get_the_title($holiday_id) : ""; 
			                        
			                    ?>
			                    <input id="holiday_id" type="hidden" name="item_id" value="<?php echo $holiday_id; ?>" data-post-type="st_holidays" class="form-control custom-form-control st_post_select_ajax " data-pl-name="<?php echo $holiday_name; ?>" data-user-id="<?php echo get_current_user_id(); ?>">
		      				</div>
	      				</div>
	      			</div>
	      			<div class="row">
	      				<div class="col-xs-12 col-sm-6">
	      					<div class="form-group">
	      						<label for="type-holiday-wrapper" class="head_bol"><?php _e('Type holiday', ST_TEXTDOMAIN); ?>:</label>
	      						<div id="type-holiday-wrapper"></div>
	      					</div>
	      				</div>
	      				<div class="col-xs-12 col-sm-6">
	      					<div class="form-group">
	      			...
	      			<div class="row">
	      				<div class="col-xs-6 col-sm-3">
	      					<div class="form-group">
	      						<a href="#" class="btn btn-primary" id="holiday_time" data-toggle="modal" data-target="#calendar_holiday_modal"><?php echo __('Select a holiday', ST_TEXTDOMAIN); ?></a>
	      					</div>
	      				</div>
	      			</div>
      				<div class="row">
      				...
      				<div class="row">
      					<div class="col-xs-12 col-sm-6">
      						<div class="form-group">
		      					<label for="check_in_holiday" class="head_bol"><?php _e('Departure date', ST_TEXTDOMAIN); ?> <span class="text-small text-danger">(*)</span>:</label>
		      					<?php 
		      						$check_in = isset($_POST['check_in']) ? $_POST['check_in'] : '';
		      					?>
		      					<div class="input-group">
								  	<span class="input-group-addon" id="basic-checkin"><i class="fa fa-calendar"></i></span>
								  	<input readonly data-date-format="<?php echo TravelHelper::getDateFormatJs() ?>" aria-describedby="basic-checkin" data-date-format="<?php echo TravelHelper::getDateFormatJs(); ?>" autocomplete="off" type="text" name="check_in" id="check_in_holiday" value="<?php echo $check_in; ?>" class="form-control">
								</div>
		      				</div>
      					</div>
      					<div class="col-xs-12 col-sm-6">
      						<div class="form-group">
		      					<label for="check_out_holiday" class="head_bol"><?php _e('Arrive date', ST_TEXTDOMAIN); ?> <span class="text-small text-danger">(*)</span>:</label>
		      					<?php 
		      						$check_out = isset($_POST['check_out']) ? $_POST['check_out'] : '';
		      					?>
		      					<div class="input-group">
			      					<span class="input-group-addon" id="basic-checkout"><i class="fa fa-calendar"></i></span>
								  	<input data-date-format="<?php echo TravelHelper::getDateFormatJs() ?>" readonly aria-describedby="basic-checkout" data-date-format="<?php echo TravelHelper::getDateFormatJs(); ?>" autocomplete="off" type="text" name="check_out" id="check_out_holiday" value="<?php echo $check_out; ?>" class="form-control">
							  	</div>
		      				</div>
      					</div>
      				</div>
