...
?>
<div class="st-create">
    <h2><?php _e("Holidays Booking" , ST_TEXTDOMAIN) ?></h2>
</div>
    <?php
    $paged = get_query_var('paged') ? intval(get_query_var('paged')) : 1;
    $limit=10;
    $offset=($paged-1)*$limit;
    $data_post=STUser_f::get_history_bookings('st_holidays',$offset,$limit,$data->ID);
...
<table class="table table-bordered table-striped table-booking-history">
    ...
    <tr>
        ...
        <th><?php _e("Holiday Name",ST_TEXTDOMAIN)?></th>
        <th><?php _e("Type Holiday",ST_TEXTDOMAIN)?></th>
        ...
        foreach( $posts as $key => $value ) {
            ...
            <tr>
                ...
                if(!empty($value->duration)){
                    $type_holiday = 'daily_holiday';
                }else{
                    $type_holiday = 'specific_date';
                }
                ?>
                <?php if($type_holiday == 'specific_date'){ ?>
                    <td class="">
    ...
    <?php
        ...
        echo '<h5>'.__('No Holiday',ST_TEXTDOMAIN).'</h5>';
    }
    ?>
    </tbody>
</table>
