...
$default=array(
    ...
    'taxonomy'=>'',
    'is_required'=>'on',
    'type_show_taxonomy_holidays'=>'checkbox'
);
...
if(!empty($terms)):
    if($type_show_taxonomy_holidays == "select"){
        ?>
        <div class="form-group form-group-<?php echo esc_attr($field_size)?>" taxonomy="<?php echo esc_html($taxonomy) ?>">
            <label for="field-holiday-tax-<?php echo esc_html($taxonomy) ?>"><?php echo esc_html( $title)?></label>
            <select id="field-holiday-tax-<?php echo esc_html($taxonomy) ?>" class="form-control" name="taxonomy[<?php echo esc_html($taxonomy) ?>]" <?php echo esc_attr($is_required) ?>>
