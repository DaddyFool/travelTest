...
echo '<h2>'.__('Holiday Booking',ST_TEXTDOMAIN).'<a href="'.admin_url('edit.php?post_type=st_holidays&page=st_holidays_booking&section=add_booking').'" class="add-new-h2">'.__('Add New',ST_TEXTDOMAIN).'</a></h2>';
    ...
    STAdmin::message();
?>
<form id="posts-filter" action="<?php echo admin_url('edit.php?post_type=st_holidays&page=st_holidays_booking')?>" method="post">
    ...
    <table class="wp-list-table widefat fixed posts">
        <thead>
            <tr>
                ...
                <th class="manage-column">
                    <a href="#"><span><?php _e('Holiday Type',ST_TEXTDOMAIN)?></span><span class="sorting-indicator"></span></a>
                </th>
                <th class="manage-column">
                        ...
                        <tr class="<?php if ($i % 2 == 0) echo 'alternate'; ?>">
                            ...
                            <td class="post-title page-title column-title">
                                <strong><a class="row-title"
                                           href="<?php echo admin_url('edit.php?post_type=st_holidays&page=st_holidays_booking&section=edit_order_item&order_item_id=' . $post_id); ?>"
                                           title=""><?php
                                        $order = $post_id;
                                        if ($order) {
                                ...
                                <div class="row-actions">
                                    <a href="<?php echo admin_url('edit.php?post_type=st_holidays&page=st_holidays_booking&section=edit_order_item&order_item_id=' . $post_id); ?>"><?php _e('Edit',ST_TEXTDOMAIN)?></a> |
                                    <a href="<?php echo admin_url('edit.php?post_type=st_holidays&page=st_holidays_booking&section=resend_email_holidays&order_item_id=' . $post_id); ?>"><?php _e('Resend Email',ST_TEXTDOMAIN)?></a>
                                </div>
                            </td>
                            <td class="post-title page-title column-title">
                                <?php
                            ...
                            if($type_holiday == 'specific_date'){ ?>
                                <td class="post-title page-title column-title">
                                    <?php _e('Specific Date',ST_TEXTDOMAIN) ?>
                                </td>
                            <?php }else{ ?>
                                <td class="post-title page-title column-title">
                                    <?php _e('Daily Holiday',ST_TEXTDOMAIN) ?>
                                </td>
                            <?php } ?>
