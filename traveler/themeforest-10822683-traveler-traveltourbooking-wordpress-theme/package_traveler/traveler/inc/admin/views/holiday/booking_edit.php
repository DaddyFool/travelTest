...
if(!isset($page_title))
{
    $page_title=__('Edit Holiday Booking',ST_TEXTDOMAIN);
}
...
?>
<div class="wrap">
    <?php echo '<h2>'.$page_title.'<a href="'.admin_url('edit.php?post_type=st_holidays&page=st_holiday_booking&section=add_booking').'" class="add-new-h2">'.__('Add New',ST_TEXTDOMAIN).'</a></h2>';?>
    <?php STAdmin::message() ?>
                    ...
                    <div id="poststuff">
                        <div class="postbox">
                            ...
                            <div class="inside">
                                ...
                                <div class="form-row">
                                    <label class="form-label" for=""><?php _e('Holiday',ST_TEXTDOMAIN)?><span class="require"> (*)</span></label>
                                    <div class="controls">
                                    <?php 

                                        $holiday_id = intval(get_post_meta($item_id,'item_id',true));
                                        if(get_post_type($holiday_id) == 'st_holidays' && $section == 'edit_order_item'):
                                    ?>
                                        <select name="item_id" id="item_id" class="form-control form-control-admin">
                                            <option value="<?php echo $holiday_id ?>"><?php echo get_the_title($holiday_id); ?></option>
                                        </select>
                                    <?php endif; ?>
                                    <?php 
                                        $holiday_id = isset($_POST['item_id']) ? intval($_POST['item_id']) : '';
                                        $holiday_name = (get_post_type($holiday_id) == 'st_holidays') ? get_the_title($holiday_id) : '';
                                        if($section == 'add_booking'):
                                    ?>
                                        <input id="holiday_id" type="hidden" name="item_id" value="<?php echo $holiday_id; ?>" data-post-type="st_holidays" class="form-control form-control-admin st_post_select_ajax " data-pl-name="<?php echo $holiday_name; ?>">
                                    <?php endif; ?>
                                    </div>
                                </div>

                                <div class="form-row">
                                    <label class="form-label" for=""><?php _e('Holiday Type',ST_TEXTDOMAIN)?></label>
                                    <div class="controls">
                                        <?php 
                                            $holiday_type = get_post_meta($item_id, 'type_holiday', true);
                                            if($holiday_type && $section == 'edit_order_item'):
                                                if($holiday_type == 'daily_holiday'){
                                                    $holiday_name = __('Daily Holiday', ST_TEXTDOMAIN);
                                                }elseif($holiday_type == 'specific_date'){
                                                    $holiday_name = __('Specific Date', ST_TEXTDOMAIN);
                                                }
                                        ?>
                                        <select readonly name="type_holiday" id="type_holiday" class="form-control form-control-admin">
                                            <option value="<?php echo $holiday_type; ?>"><?php echo $holiday_name; ?></option>
                                        </select>
                                        <?php endif; ?>
                                        <div id="holiday-type-wrapper">
                                ...
                                <div class="form-row">
                                    <label class="form-label" for="holiday_time"><?php _e('Select a holiday',ST_TEXTDOMAIN)?><span class="require"> (*)</span></label>
                                    <a class="button button-primary" id="holiday_time" href="#" onclick="return false"><?php echo __('Select', ST_TEXTDOMAIN) ?></a>
                                </div>
                                <?php endif; ?>
                                ...
                                <?php if($section != 'add_booking' && $holiday_type != 'daily_holiday'): ?>
                                <div class="form-row">
                                ...
                                <?php endif; ?>
                                <?php if($section != 'add_booking' && $holiday_type == 'daily_holiday'): $duration_unit = get_post_meta($order_item_id, 'duration_unit', true);?>
                                <div class="form-row">
...
<?php if($section == 'add_booking'): ?>
<div id="holiday_time_content" style="padding:15px; display: none">
