...
if(!class_exists( 'STUser_f' )) {
    class STUser_f extends TravelerObject
    {
        ...
        function init()
        {
            ...
            add_action( 'init' , array( $this , 'st_insert_post_type_holidays' ) , 50 );
            add_action( 'init' , array( $this , 'st_update_post_type_holidays' ) , 50 );
        ...
        function add_scripts(){
            ...
            if($_GET['sc'] == 'edit-holidays'){
                wp_enqueue_script('availability_partner.js', get_template_directory_uri() . '/js/availability_holiday_partner.js', array('jquery'), NULL, TRUE);
            }
            ...
            if($_GET['sc'] == 'add-holiday-booking'){
                wp_enqueue_script('qtip.js', get_template_directory_uri() . '/js/jquery.qtip.js', array('jquery'), NULL, TRUE);
            }
...
//                        }
        ...
        function validate_holiday(){
            ...
            if(STInput::request('holiday_type') == 'specific_date'){
                //$validator->set_rules('check_in',__("Departure Date",ST_TEXTDOMAIN),'required');
                //$validator->set_rules('check_out',__("Arrive Date",ST_TEXTDOMAIN),'required');
            }else{
                $validator->set_rules('duration',__("Duration",ST_TEXTDOMAIN),'required|unsigned_integer');
            }
            $validator->set_rules('holidays_booking_period',__("Booking Period",ST_TEXTDOMAIN),'unsigned_integer');
            $validator->set_rules('max_people',__("Max No. People",ST_TEXTDOMAIN),'required|greater_than[0]');
            if(STInput::request('st_holiday_external_booking') == 'on'){
                $validator->set_rules('st_holiday_external_booking_link',__("External Booking URL",ST_TEXTDOMAIN),'required|valid_url');
            }
            $result=$validator->run();
            if(!$result){
                STTemplate::set_message(__("Warning: Some fields must be filled in",ST_TEXTDOMAIN),'warning');
        ...
        /* Holidays */
        function st_insert_post_type_holidays()
        {
            if(!st_check_service_available('st_holidays'))
            {
                return;
            }
            if(!empty( $_REQUEST[ 'btn_insert_post_type_holidays' ] )) {
                if(wp_verify_nonce( $_REQUEST[ 'st_insert_post_holidays' ] , 'user_setting' )) {
                    if(self::validate_holiday() == false){
                        return;
                    }
                    if(st()->get_option( 'partner_post_by_admin' ) == 'on') {
                        $post_status = 'draft';
                    ...
                    if(!empty( $post_id )) {
                       ...
                       // update_post_meta( $post_id , 'price' , STInput::request( 'price' ) );
                        ...
                        update_post_meta( $post_id , 'deposit_payment_amount' , STInput::request( 'deposit_payment_amount' ) );
                        //tab info
                        update_post_meta( $post_id , 'type_holiday' , STInput::request( 'holiday_type' ) );
                        update_post_meta( $post_id , 'check_in' , STInput::request( 'check_in' ) );
                        update_post_meta( $post_id , 'check_out' , STInput::request( 'check_out' ) );
                        update_post_meta( $post_id , 'duration_day' , STInput::request( 'duration' ) );
                        update_post_meta( $post_id , 'duration_unit' , STInput::request( 'duration_unit' ) );
                        update_post_meta( $post_id , 'holidays_booking_period' , (int)STInput::request( 'holidays_booking_period' ) );
                        update_post_meta( $post_id , 'max_people' , (int)STInput::request( 'max_people' ) );
                        update_post_meta( $post_id , 'st_holiday_external_booking' , STInput::request( 'st_holiday_external_booking' ) );
                        update_post_meta( $post_id , 'st_holiday_external_booking_link' , STInput::request( 'st_holiday_external_booking_link' ) );
                        if(!empty( $_REQUEST[ 'program_title' ] )) {
                            $program_title = $_REQUEST[ 'program_title' ];
                            $program_desc  = $_REQUEST[ 'program_desc' ];
                            $program       = array();
                            ...
                            update_post_meta( $post_id , 'holidays_program' , $program );
                        }
                        /////////////////////////////////////
                        /// Update Payment
                        /////////////////////////////////////
                        ...
                        self::$msg = array(
                            'status' => 'success' ,
                            'msg'    => 'Create Holidays successfully !'
                        );
                        $page_my_account_dashboard = st()->get_option('page_my_account_dashboard');
                        if(!empty($page_my_account_dashboard)){
                            wp_redirect( add_query_arg( array('sc'=>'my-holidays','create'=>'true') , get_the_permalink($page_my_account_dashboard) ) );
                            exit;
                        }
                    } else {
                        self::$msg = array(
                            'status' => 'danger' ,
                            'msg'    => 'Error : Create Holidays not successfully !'
                        );
                    }
        ...
        /* Update Holidays */
        function st_update_post_type_holidays()
        {
            if(!st_check_service_available('st_holidays'))
            {
                return;
            }
            if(!empty( $_REQUEST[ 'btn_update_post_type_holidays' ] )) {
                if(wp_verify_nonce( $_REQUEST[ 'st_update_post_holidays' ] , 'user_setting' )) {
                    if(self::validate_holiday() == false){
                        return;
                    }
                    $post_id = STInput::request('id');
                    if(!empty( $post_id )) {
                       ...
                       // update_post_meta( $post_id , 'price' , STInput::request( 'price' ) );
                        ...
                        self::$msg = array(
                            'status' => 'success' ,
                            'msg'    => 'Update holidays successfully !'
                        );
                    } else {
                        self::$msg = array(
                            'status' => 'danger' ,
                            'msg'    => 'Error : Update holidays not successfully !'
                        );
                    }
        ...
        function get_book_history( $type = '' )
        {
            ...
            while( have_posts() ) {
                ...
                if(get_post_type( $id_item ) == 'st_holidays') {
                    $type_holiday = get_post_meta( $id_item , 'type_holiday' , true );
        ...
        function get_icon_type_order_item( $id_item )
        {
            ...
            switch( $post_type ) {
                ...
                case "st_tours":
                    ...
                    break;
                case "st_holidays":
                    $html = '<i class="fa fa-bolt"></i><small>' . __( 'holiday' , ST_TEXTDOMAIN ) . '</small>';
                    break;
                case "st_cars":
            ...
            if(!empty( $_REQUEST[ 'sc' ] )) {
                ...
                switch( $type ) {
                   ...
                   case "my-tours":
                        ...
                        break;
                    case "create-holidays":
                        st_the_language( 'user_create_holiday' );
                        break;
                    case "my-holidays":
                        st_the_language( 'user_my_holiday' );
                        break;
        ...
        */
        static function get_info_reports( $type = 'month' , $year = false , $year_start = false , $year_end = false , $_date_start = false , $_date_end = false )
        {
            ...
            if(!empty( $list_partner )) {
                foreach( $list_partner as $key => $value ) {
                    ...
                    if($value[ 'id_partner' ] == 'car' or $value[ 'id_partner' ] == 'tour' or $value[ 'id_partner' ] == 'holiday') {
        ...
        static function get_default_info_reports( $type , $date_start = false , $date_end = false )
        {
            ...
            if (!st_check_service_available('st_holidays')) { unset($data['post_type']['st_holidays']); }
        ...
        */
        ...
        static function st_get_breadcrumb_partner( )
        {
            ...
            switch($sc){
                ...
                case "booking-tours":
                    ...
                    break;
                case "create-holidays":
                    $html .= '<li>&nbsp;'.__("Add new holiday",ST_TEXTDOMAIN).'</li>';
                    break;
                case "my-holidays":
                    $html .= '<li>&nbsp;'.__("My Holiday",ST_TEXTDOMAIN).'</li>';
                    break;
                case "booking-holidays":
                    $html .= '<li>&nbsp;'.__("Holiday Bookings",ST_TEXTDOMAIN).'</li>';
                    break;
                case "create-activity":
                ...
                case "edit-tour":
                    ...
                    break;
                case "edit-holiday":
                    $html .= '<li>&nbsp;'.__("Edit Holiday ",ST_TEXTDOMAIN).'</li>';
                    break;
                case "edit-activity":
        ...
        static function st_get_title_head_partner( )
        {
            ...
            switch($sc){
                ...
                case "booking-tours":
                    ...
                    break;
                case "create-holidays":
                    $html .= __("Add new holiday",ST_TEXTDOMAIN);
                    break;
                case "my-holidays":
                    $html .= __("My Holiday",ST_TEXTDOMAIN);
                    break;
                case "booking-holidays":
                    $html .= __("Holiday Bookings",ST_TEXTDOMAIN);
                    break;
                case "create-activity":
                ...
                case "edit-tour":
                    ...
                    break;
                case "edit-holiday":
                    $html .= __("Edit Holiday ",ST_TEXTDOMAIN);
                    break;
                case "edit-activity":
        ...
        static function st_get_data_reports_total_all_time_partner( )
        {
            ...
            $data['post_type']['st_holidays']['average_total'] = 0;
            ...
            $data['post_type']['st_holidays']['total'] = 0;
            ...
            $data['post_type']['st_holidays']['number_orders'] = 0;
