jQuery(document).ready(function($) {
    ...
    var listDate = [];
    if($('input.holiday_book_date').length > 0){
        $('input.holiday_book_date').each(function(index, el) {
            $(this).datepicker({
            ...
            holiday_id = $(this).data('holiday-id');
            ajaxGetRentalOrder($this, year_start, holiday_id);
        });

        $('input.holiday_book_date').on('changeYear', function(e) {
            var $this = $(this);
            year =  new Date(e.date).getFullYear();
            holiday_id = $(this).data('holiday-id');
            ajaxGetRentalOrder( $this, year, holiday_id);
        });
    ...
    function ajaxGetRentalOrder(me, year, holiday_id){
        var data = {
            holiday_id: holiday_id,
            year: year,
            action:'st_get_disable_date_holiday',
        };
    ...
    var HolidayCalendar = function(container){
        var self = this;
            ...
            self.calendar.fullCalendar({ 
                ...
                events:function(start, end, timezone, callback) {
                    $.ajax({
                        ...
                        type:'post',
                        data: {
                            action: 'st_get_availability_holiday_frontend',
                            holiday_id: self.container.data('post-id'),
                            start: start.unix(),
                            end: end.unix()
                        },
    ...
    $('.calendar-wrapper').each(function(index, el) {
        var t = $(this);
        var holiday = new HolidayCalendar(t);
        holiday.init();
    });
    if($('#select-a-holiday').length){
        $("#select-a-holiday").fancybox({
            'transitionIn'      : 'none',
            'transitionOut'     : 'none',
        });
    }
    if($('#list_holiday_item').length){
        $('#list_holiday_item input').on('ifChecked', function(event){
            $('#check_in_2').val($(this).data('start')).parent().show();
            $('#check_out_2').val($(this).data('end'));
            if($('#list_holiday_item').data('type-holiday') != 'daily_holiday'){
